...
SoftwareSerial moveSerial(3,10);
...
void checkMove() {
  if (moveSerial.available()>0) {
      state = moveSerial.read();
      if (state=='S') {
        motor1.brake();
        motor2.brake();
      }
      else if (state=='F') {
        x+=dx*cos(radians(angle));
        y+=dx*sin(radians(angle));
        //moveSerial.println(String(x)+","+String(y)+","+String(angle)+"A");
        motor1.drive(255);
        motor2.drive(255);
      }
      else if (state=='B') {
